...
async function listEvents(event) {
  ...
  try {
    ...
    if (eventId) {
      ...
      return jsonResponse(200, data2);
    }
    const rawCountry = event.queryStringParameters?.country;
    const country = typeof rawCountry === "string" ? rawCountry.trim() : "";
    let query = supabase.from("events").select("id, name, location, banner_url, tagline, country, start_time, end_time").order("start_time", { ascending: true });
    if (country.length > 0) {
